...
```bash
$ npm i react-keyhooks --save
```
## 🔨 Usage
```jsx
// 根据需要结构以下对应模块使用
import { useKeyEvent } from 'react-keyhooks'; //键盘hooks
import { useCtrlPlusKeyEvent } from 'react-keyhooks'; //ctrl+key复合键hooks
import { emitter } from 'react-keyhooks'; //事件队列中心（单例）
```
## Demo
